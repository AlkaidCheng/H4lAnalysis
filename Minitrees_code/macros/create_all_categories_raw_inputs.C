...
void create_all_categories_raw_inputs(TString Input_file = "../inputs/mc15_ggH125_30.root",  TString TTree_input = "newtree", TString Output_file = "outputs/output_category_2jet_VH_ggH125_nominal.root",TString Histo_Output_Name="ggH125_BDT", TString var_1 ="BDT_VH_discriminant", TString var_2 ="BDT_VH_discriminant", int const number_of_bin = 10 ,float lower_cut = -1., float upper_cut = 1., float Luminosity_fb = 10., TString cat = "2jet_VH", TString sample = "ggH125"){
bool Raw = false;
      ...
      if(discriminant>=lower_cut && discriminant<=upper_cut ){
	    ...
	    if(Raw){
		 ...
		 Luminosity_fb = 1.;
            ...
            if ( (cat=="ttH")&&(prod_type==5) ) histo_output->Fill( discriminant,1);

// 3 pt4l categories for 1 jet
            if ( prod_type==1 ){
                if(cat=="1jet_highpt4l" && pt4l_fsr > 120.) histo_output->Fill( discriminant,1);
                if(cat=="1jet_lowpt4l" && pt4l_fsr <= 60.) histo_output->Fill( discriminant,1);
                if(cat=="1jet_mediumpt4l" && pt4l_fsr >60 && pt4l_fsr <=120.) histo_output->Fill( discriminant,1);
		
	    }
// 2 pt4l categories for VBF
            if ( prod_type==3 ){
                if(cat=="2jet_VBF_highpt4l" && pt4l_fsr > 150.) histo_output->Fill( discriminant,1);
                if(cat=="2jet_VBF_lowpt4l" && pt4l_fsr <= 150.) histo_output->Fill( discriminant,1);
            }
// 2 pt4l categories for VH
            if ( prod_type==2 ){
                if(cat=="2jet_VH_highpt4l" && pt4l_fsr > 150.) histo_output->Fill( discriminant,1);
                if(cat=="2jet_VH_lowpt4l" && pt4l_fsr <= 150.) histo_output->Fill( discriminant,1);
            }
// 2 pt jet categories for VBF
            if ( prod_type==3 ){
                if(cat=="2jet_VBF_highptjet" && leading_jet_pt > 200.) histo_output->Fill( discriminant,1);
                if(cat=="2jet_VBF_lowptjet" && leading_jet_pt <= 200.) histo_output->Fill( discriminant,1);
            }
// 2 pt jet categories for VH
            if ( prod_type==2 ){
                if(cat=="2jet_VH_highptjet" && leading_jet_pt > 150.) histo_output->Fill( discriminant,1);
                if(cat=="2jet_VH_lowptjet" && leading_jet_pt <= 150.) histo_output->Fill( discriminant,1);
            }
