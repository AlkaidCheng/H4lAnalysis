...
void writeXML_ggf_only(bool add_syst = true, bool add_stat_err = false, bool theo_syst = false,TString sample_names_file = "macros/samples_SM.txt", TString jetptcut = "35" , bool theo_syst_EFT = false , TString syst_names_file = "macros/systematics.txt"){
    ...
    if (theo_syst) std::cout<<"ADDING THEORETICAL SYSTEMATIC ERRORS"<<std::endl;
	  ...
	  if(!skip_syst){
           for(int m = 0 ; m<systN; m++){
            ...
            if(systematics[m]!="JET_JER_SINGLE_NP"  && systematics[m]!="weight_var_JvtEfficiency" && systematics[m]!="MUONS_TTVA__byHand"){// && systematics[m]!="weight_JvtEfficiencyUp"){
              	fileName_UP = "outputs"+region[j]+"/output_category_"+ region[j]+ "_"+ sample[k] + "_" + systematics[m]+"__1up.root";
//"outputs/output_category_"
              	...
              	fileName_DOWN = "outputs"+region[j]+"/output_category_"+ region[j]+ "_"+ sample[k] + "_" + systematics[m]+"__1down.root";
...
//fixing syst fixed to 1
		if(var_up>0.99999 && var_up<1.000001 && var_down<=1.0) var_up = 1.00001;
                if(var_up>0.99999 && var_up<1.000001 && var_down>1.0) var_up = 0.99999;
                if(var_down>0.9999 && var_down<1.00001 && var_up<=1.0) var_down = 1.00001; 
                if(var_down>0.9999 && var_down<1.00001 && var_up>1.0) var_down = 0.99999; 

//pruning by hand

                if( !((var_up)<1.005 && (var_up)>0.995) || !((var_down)<1.005 && (var_down)>0.995) ) {
// before I had value > 0.01
                if(value > 0.0001 && ((var_up - 1. )*(var_down - 1.))<0 ) 	*optfile<<"    <OverallSys Name=\"" << systematics[m] << "\" High= \""<< var_up <<"\" Low=\"" << var_down << "\"/>" << std::endl;
		}
            }
	    if(systematics[m]=="JET_JER_SINGLE_NP" /*|| systematics[m]=="NormSystematic"*/){
		
		if (systematics[m]=="JET_JER_SINGLE_NP") fileName_UP = "outputs"+region[j]+"/output_category_"+ region[j]+ "_"+ sample[k] + "_" + systematics[m]+"__1up.root";
                if (systematics[m]=="NormSystematic") fileName_UP = "outputs"+region[j]+"/output_category_"+ region[j]+ "_"+ sample[k] + "_" + systematics[m]+".root";
                TFile *f_input_UP = TFile::Open(fileName_UP);
                if(!f_input_UP) continue;
                TH1F *h_signal_UP = (TH1F*)f_input_UP->Get(histoName);
                value_UP = h_signal_UP->GetBinContent(1);

                if (systematics[m]=="JET_JER_SINGLE_NP") fileName_DOWN = "outputs"+region[j]+"/output_category_"+ region[j]+ "_"+ sample[k] + "_" + systematics[m]+"__1up.root";
                if (systematics[m]=="NormSystematic") fileName_DOWN = "outputs"+region[j]+"/output_category_"+ region[j]+ "_"+ sample[k] + "_" + systematics[m]+".root";
                std::cout << "file = " << fileName_DOWN << std::endl;
...
//weight_JvtEfficiencyUP
		...
		fileName_UP = "outputs"+region[j]+"/output_category_"+ region[j]+ "_"+ sample[k] + "_" + systematics[m]+"Up.root";
                TFile *f_input_UP = TFile::Open(fileName_UP);
                if(!f_input_UP) continue;
                TH1F *h_signal_UP = (TH1F*)f_input_UP->Get(histoName);
                value_UP = h_signal_UP->GetBinContent(1);
                fileName_DOWN = "outputs"+region[j]+"/output_category_"+ region[j]+ "_"+ sample[k] + "_" + systematics[m]+"Down.root";
                std::cout << "file = " << fileName_DOWN << std::endl;
                TFile *f_input_DOWN = TFile::Open(fileName_DOWN);
                if(!f_input_DOWN)continue;
                TH1F *h_signal_DOWN = (TH1F*)f_input_DOWN->Get(histoName);
...
*/
	...
	//	*optfile <<"    <NormFactor Name=\"mu_vbf\" Val=\"1.\" Low=\"0.\" High=\"20.\" Const=\"True\" />" << std::endl;
		...
		if(region[j]=="0jet") *optfile <<"    <OverallSys Name=\"ATLAS_UEPS_qqH\" High=\"0.9824\" Low=\"1.0155\" />" << std::endl;
	      ...
	      }//closing Higgs theo systematics
...
//		*optfile <<"    <OverallSys Name=\"ATLAS_BR_VV\" High=\"1.0264\" Low=\"0.974\" />" << std::endl;
		...
		*optfile <<"    <NormFactor Name=\"mu_vh\" Val=\"1.\" Low=\"0.\" High=\"90.\" Const=\"True\" />" << std::endl;
		...
		if(theo_syst){
/* ST vertical */
// theo syst from David 17-3-2017
// NO PDF
/*1) ATLAS_QCDscale_ggH_pt_m*/
                   if(region[j]=="0jet") *optfile <<"    <OverallSys Name=\"ATLAS_QCDscale_ggH_pt_m\" High=\"1.002192\" Low=\"0.997808\" />" << std::endl;
                   if(region[j]=="1jet_lowpt4l") *optfile <<"    <OverallSys Name=\"ATLAS_QCDscale_ggH_pt_m\" High=\"1.027824\" Low=\"0.972176\" />" << std::endl;
                   if(region[j]=="1jet_mediumpt4l") *optfile <<"    <OverallSys Name=\"ATLAS_QCDscale_ggH_pt_m\" High=\"0.971327\" Low=\"1.028673\" />" << std::endl;
                   if(region[j]=="1jet_highpt4l") *optfile <<"    <OverallSys Name=\"ATLAS_QCDscale_ggH_pt_m\" High=\"0.863317\" Low=\"1.136683\" />" << std::endl;



                   if(region[j]=="2jet_VH_highpt4l") *optfile <<"    <OverallSys Name=\"ATLAS_QCDscale_ggH_pt_m\" High=\"0.880142\" Low=\"1.119858\" />" << std::endl;
                   if(region[j]=="2jet_VH_lowpt4l") *optfile <<"    <OverallSys Name=\"ATLAS_QCDscale_ggH_pt_m\" High=\"1.002987\" Low=\"0.997013\" />" << std::endl;

                   
                   if(region[j]=="2jet_VBF_lowpt4l") *optfile <<"    <OverallSys Name=\"ATLAS_QCDscale_ggH_pt_m\" High=\"0.999370\" Low=\"1.000630\" />" << std::endl;
                   if(region[j]=="2jet_VBF_highptjet") *optfile <<"    <OverallSys Name=\"ATLAS_QCDscale_ggH_pt_m\" High=\"0.844296\" Low=\"1.155704\" />" << std::endl;

                  if(region[j]=="VHlep") *optfile <<"    <OverallSys Name=\"ATLAS_QCDscale_ggH_pt_m\" High=\"0.993972\" Low=\"1.006028\" />" << std::endl;
                   if(region[j]=="ttH") *optfile <<"    <OverallSys Name=\"ATLAS_QCDscale_ggH_pt_m\" High=\"0.946800\" Low=\"1.053200\" />" << std::endl;




/* 2) ATLAS_QCDscale_ggF_H4l
 * */                 
                   if(region[j]=="0jet") *optfile <<"    <OverallSys Name=\"ATLAS_QCDscale_ggF_H4l\" High=\"0.961256\" Low=\"1.038744\" />" << std::endl;
                   if(region[j]=="1jet_lowpt4l") *optfile <<"    <OverallSys Name=\"ATLAS_QCDscale_ggF_H4l\" High=\"0.952323\" Low=\"1.047677\" />" << std::endl;
                   if(region[j]=="1jet_mediumpt4l") *optfile <<"    <OverallSys Name=\"ATLAS_QCDscale_ggF_H4l\" High=\"0.946167\" Low=\"1.053833\" />" << std::endl;
                   if(region[j]=="1jet_highpt4l") *optfile <<"    <OverallSys Name=\"ATLAS_QCDscale_ggF_H4l\" High=\"0.944378\" Low=\"1.055622\" />" << std::endl;
                   if(region[j]=="2jet_VH_lowpt4l") *optfile <<"    <OverallSys Name=\"ATLAS_QCDscale_ggF_H4l\" High=\"0.930315\" Low=\"1.069685\" />" << std::endl;
                   if(region[j]=="2jet_VH_highpt4l") *optfile <<"    <OverallSys Name=\"ATLAS_QCDscale_ggF_H4l\" High=\"0.919116\" Low=\"1.080884\" />" << std::endl;
                   if(region[j]=="2jet_VBF_lowptjet") *optfile <<"    <OverallSys Name=\"ATLAS_QCDscale_ggF_H4l\" High=\"0.926087\" Low=\"1.073913\" />" << std::endl;
                   if(region[j]=="2jet_VBF_highptjet") *optfile <<"    <OverallSys Name=\"ATLAS_QCDscale_ggF_H4l\" High=\"0.918258\" Low=\"1.081742\" />" << std::endl;
                   if(region[j]=="VHlep") *optfile <<"    <OverallSys Name=\"ATLAS_QCDscale_ggF_H4l\" High=\"0.945747\" Low=\"1.054253\" />" << std::endl;
                   if(region[j]=="ttH") *optfile <<"    <OverallSys Name=\"ATLAS_QCDscale_ggF_H4l\" High=\"0.917775\" Low=\"1.082225\" />" << std::endl;

/* 3) ATLAS_QCDres_ggF
 * */                   
                   if(region[j]=="0jet") *optfile <<"    <OverallSys Name=\"ATLAS_QCDres_ggF\" High=\"0.996396\" Low=\"1.003604\" />" << std::endl;
                   if(region[j]=="1jet_lowpt4l") *optfile <<"    <OverallSys Name=\"ATLAS_QCDres_ggF\" High=\"0.969791\" Low=\"1.030209\" />" << std::endl;
                   if(region[j]=="1jet_mediumpt4l") *optfile <<"    <OverallSys Name=\"ATLAS_QCDres_ggF\" High=\"0.953414\" Low=\"1.046586\" />" << std::endl;
                   if(region[j]=="1jet_highpt4l") *optfile <<"    <OverallSys Name=\"ATLAS_QCDres_ggF\" High=\"0.950501\" Low=\"1.049499\" />" << std::endl;
                   if(region[j]=="2jet_VH_lowpt4l") *optfile <<"    <OverallSys Name=\"ATLAS_QCDres_ggF\" High=\"0.935657\" Low=\"1.064343\" />" << std::endl;
                   if(region[j]=="2jet_VH_highpt4l") *optfile <<"    <OverallSys Name=\"ATLAS_QCDres_ggF\" High=\"0.919718\" Low=\"1.080282\" />" << std::endl;
                   if(region[j]=="2jet_VBF_lowptjet") *optfile <<"    <OverallSys Name=\"ATLAS_QCDres_ggF\" High=\"0.929759\" Low=\"1.070241\" />" << std::endl;
                   if(region[j]=="2jet_VBF_highptjet") *optfile <<"    <OverallSys Name=\"ATLAS_QCDres_ggF\" High=\"0.918674\" Low=\"1.081326\" />" << std::endl;
                   if(region[j]=="VHlep") *optfile <<"    <OverallSys Name=\"ATLAS_QCDres_ggF\" High=\"0.961073\" Low=\"1.038927\" />" << std::endl;
                   if(region[j]=="ttH") *optfile <<"    <OverallSys Name=\"ATLAS_QCDres_ggF\" High=\"0.918331\" Low=\"1.081669\" />" << std::endl;
/* 4) ATLAS_QCDscale_ggH_m01
 * */

                   if(region[j]=="0jet") *optfile <<"    <OverallSys Name=\"ATLAS_QCDscale_ggH_m01\" High=\"1.034499\" Low=\"0.965501\" />" << std::endl;
                   if(region[j]=="1jet_lowpt4l") *optfile <<"    <OverallSys Name=\"ATLAS_QCDscale_ggH_m01\" High=\"0.965825\" Low=\"1.034175\" />" << std::endl;
                   if(region[j]=="1jet_mediumpt4l") *optfile <<"    <OverallSys Name=\"ATLAS_QCDscale_ggH_m01\" High=\"0.928297\" Low=\"1.071703\" />" << std::endl;
                   if(region[j]=="1jet_highpt4l") *optfile <<"    <OverallSys Name=\"ATLAS_QCDscale_ggH_m01\" High=\"0.926644\" Low=\"1.073356\" />" << std::endl;
                   if(region[j]=="2jet_VH_lowpt4l") *optfile <<"    <OverallSys Name=\"ATLAS_QCDscale_ggH_m01\" High=\"0.953947\" Low=\"1.046053\" />" << std::endl;
                   if(region[j]=="2jet_VH_highpt4l") *optfile <<"    <OverallSys Name=\"ATLAS_QCDscale_ggH_m01\" High=\"0.955072\" Low=\"1.044928\" />" << std::endl;
                   if(region[j]=="2jet_VBF_lowptjet") *optfile <<"    <OverallSys Name=\"ATLAS_QCDscale_ggH_m01\" High=\"0.954969\" Low=\"1.045031\" />" << std::endl;
                   if(region[j]=="2jet_VBF_highptjet") *optfile <<"    <OverallSys Name=\"ATLAS_QCDscale_ggH_m01\" High=\"0.956038\" Low=\"1.043962\" />" << std::endl;
                   if(region[j]=="VHlep") *optfile <<"    <OverallSys Name=\"ATLAS_QCDscale_ggH_m01\" High=\"0.969692\" Low=\"1.030308\" />" << std::endl;
                   if(region[j]=="ttH") *optfile <<"    <OverallSys Name=\"ATLAS_QCDscale_ggH_m01\" High=\"0.957818\" Low=\"1.042182\" />" << std::endl;
/* 5) ATLAS_QCDscale_ggH_m12
 * */

                   if(region[j]=="0jet") *optfile <<"    <OverallSys Name=\"ATLAS_QCDscale_ggH_m12\" High=\"1.003297\" Low=\"0.996703\" />" << std::endl;
                   if(region[j]=="1jet_lowpt4l") *optfile <<"    <OverallSys Name=\"ATLAS_QCDscale_ggH_m12\" High=\"1.035315\" Low=\"0.964685\" />" << std::endl;
                   if(region[j]=="1jet_mediumpt4l") *optfile <<"    <OverallSys Name=\"ATLAS_QCDscale_ggH_m12\" High=\"1.045157\" Low=\"0.954843\" />" << std::endl;
                   if(region[j]=="1jet_highpt4l") *optfile <<"    <OverallSys Name=\"ATLAS_QCDscale_ggH_m12\" High=\"1.036465\" Low=\"0.963535\" />" << std::endl;
                   if(region[j]=="2jet_VH_lowpt4l") *optfile <<"    <OverallSys Name=\"ATLAS_QCDscale_ggH_m12\" High=\"0.917889\" Low=\"1.082111\" />" << std::endl;
                   if(region[j]=="2jet_VH_highpt4l") *optfile <<"    <OverallSys Name=\"ATLAS_QCDscale_ggH_m12\" High=\"0.849185\" Low=\"1.150815\" />" << std::endl;
                   if(region[j]=="2jet_VBF_lowptjet") *optfile <<"    <OverallSys Name=\"ATLAS_QCDscale_ggH_m12\" High=\"0.891207\" Low=\"1.108793\" />" << std::endl;
                   if(region[j]=="2jet_VBF_highptjet") *optfile <<"    <OverallSys Name=\"ATLAS_QCDscale_ggH_m12\" High=\"0.842827\" Low=\"1.157173\" />" << std::endl;
                   if(region[j]=="VHlep") *optfile <<"    <OverallSys Name=\"ATLAS_QCDscale_ggH_m12\" High=\"0.990975\" Low=\"1.009025\" />" << std::endl;
                   if(region[j]=="ttH") *optfile <<"    <OverallSys Name=\"ATLAS_QCDscale_ggH_m12\" High=\"0.837700\" Low=\"1.162300\" />" << std::endl;
...
//            *optfile<<"    <OverallSys Name=\"ATLAS_MUON_TTVA_SYS\" High= \"1.01\" Low=\"0.99\"/>" << std::endl;
	  if(add_syst) *optfile<<"  <OverallSys Name=\"ATLAS_luminoisty\" High= \"1.037\" Low=\"0.963\"/>"<<std::endl;
