...
void Likelihood_scan_New_Model_ggf_Khgg_floating(TString wsInput = "output_combined_datastat_model.root" , TString workspace="combined" , TString model="ModelConfig" , TString dataName="obsData" /*"obsData" or "asimovData"*/, float coupling_1 = 1, float coupling_2 = 2 , TString formula_1 ="@1 + @0", TString file_readout = "function.txt", int number_of_base = 3, TString coupling_name_file = "coupling_name.txt", bool total_width_fix = true, bool ggf_BR_modification = true, const float inizio = -8, const float larghezza = 0.1, const int passi = 160 , bool total_mu_ggf = false){
...
// bool total_width_fix = true, bool ggf_BR_modification = true
    ...
    g_sign->GetXaxis()->SetTitle(coupling_name[1]);
    g_sign->GetXaxis()->SetTitleSize(0.3);
    g_sign->GetYaxis()->SetTitle("-2DLL");
    g_sign->GetYaxis()->SetTitleSize(0.3);
    g_sign->SaveAs("output/"+file_name_string+".eps");
    g_sign->SaveAs("output/"+file_name_string+".pdf");
