...
void create_Coupling_Analysis_inputs(TString Input_file = "../inputs/mc15_ggH125_30.root",  TString TTree_input = "newtree", TString Output_file = "outputs/output_category_2jet_VH_ggH125_nominal.root",TString Histo_Output_Name="ggH125_BDT", TString var_1 ="BDT_VH_discriminant", TString var_2 ="BDT_VH_discriminant", int const number_of_bin = 10 ,float lower_cut = -1., float upper_cut = 1., float Luminosity_fb = 10., TString cat = "2jet_VH", TString sample = "ggH125"){
bool Raw = false;
  ...
  tree_input->SetBranchAddress("pt4l_unconstrained",&pt4l_unconstrained);
  tree_input->SetBranchAddress("dijet_deltaeta",&dijet_deltaeta);
/*
...
*/
  ...
  if(cat!="VHlep") tree_input->SetBranchAddress(var_1,&variable_1);
  tree_input->SetBranchAddress("m4l_constrained",&m4l_constrained);
...
/*    
  ...
  */
  ...
  for (Int_t i=0; i<tree_input_nentries; i++) {
      tree_input->GetEntry(i);
      discriminant = 1;
      if ( cat!="VHlep" ) discriminant = variable_1;
...
*/
	    if(Raw){
		 ...
		 Luminosity_fb = 1.;
            ...
            if ( (cat=="0jet")&&(prod_type==0) ) histo_output->Fill( discriminant,Luminosity_fb*(weight/w_lumi));
            if ( (cat=="1jet_lowpt4l" || cat=="1jet_highpt4l")&&(prod_type==1) ) {
			if(pt4l_unconstrained<=120 && cat=="1jet_lowpt4l"){
				histo_output->Fill( discriminant,Luminosity_fb*(weight/w_lumi));
			}
                        if(pt4l_unconstrained>120 && cat=="1jet_highpt4l"){
                                histo_output->Fill( discriminant,Luminosity_fb*(weight/w_lumi));
                        }
            ...
            if ( (cat=="2jet_VH")&&(prod_type==2) ) histo_output->Fill( discriminant,Luminosity_fb*(weight/w_lumi));
            if ( (cat=="2jet_VBF_lowmjj" || cat=="2jet_VBF_highmjj")&&(prod_type==3) ){
                        if((dijet_invmass<=400 || dijet_deltaeta<=2.8) && cat=="2jet_VBF_lowmjj"){
				histo_output->Fill( discriminant,Luminosity_fb*(weight/w_lumi));
			}
                        if(dijet_invmass>400 && dijet_deltaeta>2.8 && cat=="2jet_VBF_highmjj"){
                                histo_output->Fill( discriminant,Luminosity_fb*(weight/w_lumi));
                        }
