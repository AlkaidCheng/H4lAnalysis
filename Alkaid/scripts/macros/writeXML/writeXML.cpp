...
int H4l_XML::writeXML(TString ErrorType, TString sample_in, TString category_in, TString norm_in, TString syst_in, TString jetptcut,TString discriminant, TString outpath , const bool &IsAsimov) {
	...
	for (auto &category : Category) {
		...
		for (auto &sample : Sample) {
			...
			if ((category == "VHlep" || category == "ttH" || category == "2jet_VBF_highptjet") && (sample.Contains("gfMG") && sample != "ggfMG5_0p"))
				continue;
      ...
      if (!h_signal){
        ...
        if ((!h_up)||(!h_down)){
          ...
          continue;
				...
				if (!((N_up)<1.01 && (N_up)>0.99) || !((N_down)<1.01 && (N_down)>0.99))
					if (signal_norm > 0.0001 && ((N_up - 1.)*(N_down - 1.))<0)
						outfile << "    <OverallSys Name=\"" << syst << "\" High= \"" << N_up << "\" Low=\"" << N_down << "\"/>" << std::endl;
			}  //End of Syst

			   //Adding Normalization Factor
			TString NormFactor = SysMatch(sample, category, NormFactor_vector, "NormFactor Name");
