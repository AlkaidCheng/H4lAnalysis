  ...
  for category in histograms_by_category:
   ...
   if "STACK" in Work["SETTING"]:
    ...
    for h in histograms_by_category[category]:
     if (histograms_by_category[category][h].Integral() == 0):  #Avoid Including Empty Categories
      continue
     PlotUtil.kinematics_plot(histograms_by_category[category][h],title = "{0}_{1}".format(h,param["NAME"]),xtitle = param["NAME"],ytitle = "Event", canvasstyle = param["CANVASSTYLE"],normalize = param["NORMALIZATION"],style = param["STYLE"], drawstyle = param["DRAWSTYLE"],output = join(outdir,"{0}.pdf".format(Work["OUTPUTNAME_BYCATEGORY"].replace("PARAM",param["NAME"]).replace("PROCESS",h).replace("CATEGORY",category))), ymargin = 0.35, saveROOT = ("SAVEROOT" in Work["SETTING"]))