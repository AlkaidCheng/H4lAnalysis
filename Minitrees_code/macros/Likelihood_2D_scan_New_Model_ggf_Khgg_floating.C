...
void Likelihood_2D_scan_New_Model_ggf_Khgg_floating(TString wsInput = "output_combined_datastat_model.root" , TString workspace="combined" , TString model="ModelConfig" , TString dataName="obsData" /*"obsData" or "asimovData"*/, float coupling_1 = 1, float coupling_2 = 2 , TString formula_1 ="@1 + @0", TString file_readout = "function.txt", int number_of_base = 3, TString coupling_name_file = "coupling_name.txt", bool total_width_fix = true, bool ggf_BR_modification = true, const float inizio = -8, const float larghezza = 0.1, const int passi = 160 , bool total_mu_ggf = false, const float starting_point_2 = -8, const float width_2 = 0.1, const int scan_point_2 = 6 ){
//with the last 3 arguments you are setting the Y axis that is the SM case.
...
*/
    ...
    float intro_x = inizio - (larghezza/2.);
    float fine_x = inizio + larghezza * passi - (larghezza/2.);

    float intro_y = starting_point_2 - (width_2/2.) ;
    float fine_y = starting_point_2 + width_2 * scan_point_2 - (width_2/2.) ;


    TH2F *h2f = new TH2F("h2f","Observed Distribution",passi,intro_x,fine_x,scan_point_2,intro_y,fine_y);
    TH1F *h1f_kagg = new TH1F("h1f_kagg","Distribution Kagg" ,passi,intro_x,fine_x);
...
//likelihood uncond
    ...
    test_prova_sm->setVal(1);
    test_prova_sm->setConstant(false);
...
*/
        for(int jj = 0 ; jj< scan_point_2 ; jj++){

		Likelihood = -999.;
		coupl_val = starting_point + i*width;
		coupl_2_val = starting_point_2 + jj*width_2;

		test_prova->setVal(coupl_val);
		test_prova->setConstant(true);
	    	test_prova_sm->setVal(coupl_2_val);
	    	test_prova_sm->setConstant(true);
		...
		Likelihood_scan[i]=m2DLL;
                ...
                std::cout<< "2DLL = " << m2DLL <<"   at a coupl_val " << test_prova->getVal() << "  and  coupl_2_val" << test_prova_sm->getVal() << std::endl;

	        SM_value = test_prova_sm->getVal();
	       	BSM_value = test_prova->getVal();

                h2f->Fill(BSM_value, SM_value, m2DLL);
                if(SM_value==1) h1f_kagg->Fill(BSM_value,m2DLL);

		
		tree_scan->Fill();
		delete nll_cond;
       }//Here I closed the Khgg loop
    }//Here it closes the loop over KAgg
...
// bool total_width_fix = true, bool ggf_BR_modification = true
    ...
    h2f->GetXaxis()->SetTitle(coupling_name[1]);
    h2f->GetYaxis()->SetTitle(coupling_name[0]);


    h1f_kagg->GetXaxis()->SetTitle(coupling_name[1]);
    h1f_kagg->GetYaxis()->SetTitle("-2DLL");
